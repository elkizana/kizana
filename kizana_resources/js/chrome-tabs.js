((window, factory) => {
  ...
  const tabTemplate = `
    <div class="chrome-tab" >
      ...
      <div class="chrome-tab-background">
        <svg version="1.1" xmlns="http://www.w3.org/2000/svg"><defs><symbol id="chrome-tab-geometry-right" viewBox="0 0 214 36"><path d="M17 0h197v36H0v-2c4.5 0 9-3.5 9-8V8c0-4.5 3.5-8 8-8z"/></symbol><symbol id="chrome-tab-geometry-left" viewBox="0 0 214 36"><use xlink:href="#chrome-tab-geometry-right"/></symbol><clipPath id="crop"><rect class="mask" width="100%" height="100%" x="0"/></clipPath></defs><svg width="52%" height="100%"><use xlink:href="#chrome-tab-geometry-right" width="214" height="36" class="chrome-tab-geometry"/></svg><g transform="scale(-1, 1)"><svg width="52%" height="100%" x="-100%" y="0"><use xlink:href="#chrome-tab-geometry-left" width="214" height="36" class="chrome-tab-geometry"/></svg></g></svg>
      </div>
  ...
  class ChromeTabs {
    ...
    setupEvents() {
      ...
      /* this.el.addEventListener('dblclick', event => {
        i++ ; 
        if ([this.el, this.tabContentEl].includes(event.target)) this.addTab({title : i.toString() , id : i.toString() })
      }) */
 ...
 /*    setTabCloseEventListener(tabEl) {
    ...
    updateTab(tabEl, tabProperties) {
      ...
      //tabEl.querySelector('.chrome-tab-title').id = tabProperties.id
      ...
      if (tabProperties.id) {
        ...
        closeEl.setAttribute('id', "k" +  tabProperties.id  )
      }
    }
    ...
    setupDraggabilly() {
      ...
      tabEls.forEach((tabEl, originalIndex) => {
        ...
        draggabilly.on('dragEnd', _ => {
          this.isDragging = false
          const finalTranslateX = parseFloat(tabEl.style.right, 10)
